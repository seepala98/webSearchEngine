...
public class SortedPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
    ...
    */
    public void print() throws IOException{
        ...
        Iterator<Entry<K, V>> s = list.iterator();
        while(s.hasNext()) {
            Entry<K, V> s1 = s.next();
            K score = s1.getKey();
            String str = (String) s1.getValue();
            sc.add((Integer) score);
            filename.add(str);
        }
        int pages = sc.size();
        if(pages != 0) {
            System.out.println("Found:"+ pages + "pages:  Ranking...");
            for(int i = sc.size()-1; i >= 0; i--) {
                // list occurrence of that word in decreasing order
                System.out.println(sc.get(i) +"  "+ filename.get(i));
            }
        }
        //	  return s1;
    }
